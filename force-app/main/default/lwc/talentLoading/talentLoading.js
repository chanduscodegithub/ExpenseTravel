...
import getUserInfo from '@salesforce/apex/saveTalentLoading.getUserInfo';
import getRecordId from '@salesforce/apex/saveTalentLoading.getRecordId';
...
import PROSTART_DATE_FIELD from '@salesforce/schema/Project__c.StartDate__c';
import PROEND_DATE_FIELD from '@salesforce/schema/Project__c.EndDate__c';
import PROBUDGET_FIELD from '@salesforce/schema/Project__c.Total_Support_Hours__c';
import PROVERSION_FIELD from '@salesforce/schema/Project__c.TalentloadVersion__c';
import PROTYPE_FIELD from '@salesforce/schema/Project__c.Type__c';
...
export default class CustomDatatableDemo extends NavigationMixin(LightningElement) {
    ...
    @track isDisabledAccess = true;

    connectedCallback() {
        //console.log('rerecordIdcID::' + this.recordId);
        this.recordIdObj = this.recordId;
        this.tableData = [];

        getUserInfo({})
            .then(result => {
                console.log('result::' + result);
                this.isDisabledAccess = result;
            }).catch(error => {
                console.log('error::' + error);
                this.isDisabledAccess = true;
            })

        getRecordId({ recId: this.recordId })
            .then(result => {
                        ...
                        if (this.frequency == 'Monthly' || this.profrequency == 'Monthly' || this.talentFrequency == 'Monthly') {
                            ...
                            const newRow = {
                                ...
                                monthsData: this.months.reduce((data, month) => {
                                    ...
                                    data.push({ month: month, value: line ? line.Hour__c : 0 });
                                    return data;
                                }, []),
                            };
    ...
    saveAction() {
        ...
        // if any of the talent Loading records is Mapped to Team Allocation it shouldn't allow to save. Show a toast message
        ...
        for (let i = 0; i < this.tableData.length; i++) {
            ...
            //console.log('row.tableData>>>>' + JSON.stringify(row));
            const role = row.column1;
            const location = row.column2;

            if (!(role && location)) {
                noRecords = noRecords + 0;
            }
            ...
            //console.log('talentLoadings>>>>' + JSON.stringify(talentLoadings));
            ...
            //console.log('talentLoadingLines>>>>' + JSON.stringify(talentLoadingLines));
        ...
        if (noRecords === 0) {
            ...
            const toastEvent = new ShowToastEvent({
                title: 'Error',
                message: 'Role and Location cannot be blank, Fill atleast one row.',
                variant: 'error',
            });
            this.dispatchEvent(toastEvent);
            return;
        ...
        createTalentRecords({ talentLoadingRecords: JSON.stringify(talentLoadings), talentLoadingLineRecords: JSON.stringify(talentLoadingLines), engId: this.recordId, start: this.startDate, endd: this.endDate })
            .then(result => {
